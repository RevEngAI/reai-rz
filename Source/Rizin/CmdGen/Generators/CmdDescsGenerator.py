    ...
    def _set_subcommands(self, c, yamls):
        if "subcommands" in c and isinstance(c["subcommands"], list):
            ...
            if subcommands_name not in yamls:
                print(
                    "Command %s referenced another YAML file (%s) that is not passed as arg to CmdDescsGenerator.py."
...
with open(os.path.join(args.output_dir, "Output/CmdDescs.c"), "w", encoding="utf8") as f:
    f.write(cf_text)
if args.src_output_dir:
    with open(
        os.path.join(args.src_output_dir, "Output/CmdDescs.c"), "w", encoding="utf8"
    ) as f:
        f.write(cf_text)
...
if args.src_output_dir:
    with open(
        os.path.join(args.src_output_dir, "Output/CmdDescs.h"), "w", encoding="utf8"
    ) as f:
        f.write(hf_text)
